...
初期セットアップではサイドバーの直下に入ります。あとから設定で取り込むと「Notion から」の下に入ります。リレーション、インライン DB、カバー、画像アイコン、コメント、列レイアウト、PDF/動画、ステータス、マルチセレクト、数式は取り込みます。権限の共有設定とカスタムの複雑な数式は、Notion API では取れない／言語が違うため完全には来ません。
