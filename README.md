...
ログイン画面から常に「アカウントを作成」へ進めます。

- その環境にワークスペースがまだない → 最初のユーザーがオーナーになり、ワークスペースを作る
- すでにワークスペースがある → 招待コードが必要（公開の誰でも登録はしない）

招待 URL は `/signup?invite=<token>` です。古い `/setup` と `/invite/:token` は `/signup` にリダイレクトします。
